...
def Filter(char_chain):
    ...
    char_chain = char_chain.replace("\\",colored("\\", color="green"))
    char_chain = char_chain.replace("S",colored("S", color="green"))
...
while rejouer == True:
                ...
                if (isx, isy) == proj_pos:
                    ...
                    if each_char == "U" or current_pos == win_pos:  # ou toute autre condition de victoire
                        ...
                        if each_char in ["°","x","o"]:
                            ...
                            if each_char == "x":
                                score += 2
                            snake_body = snake_growth(snake_body)
                        player_char = facing(player_char)
                        snake_body = snake_slide(snake_body)
                        base_ref = list(fond)
                        dp_count = 0
                        for sz in range(0,len(snake_body)):
                            if dp_count <= 1 :
                                base_ref[(snake_body[sz])] = "O"
                                dp_count += 1
                            elif skin == True:
                                base_ref[(snake_body[sz])] = "/"
                                skin = False
                            else:
                                base_ref[(snake_body[sz])] = "\\"
                                skin = True
    ...
    if p_input[0] != "q":
        ...
        if reponce not in ["y", "oui", "o", "yes"]:
            ...
            snake_body = [0 for x in snake_body]
    else:
        rejouer = False